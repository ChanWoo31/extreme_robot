...
class DynamixelBusNode(Node):
    def __init__(self):
        super().__init__('dynamixel_bus_node')

        # 아두이노 시리얼 연결
        self.arduino = serial.Serial('/dev/ttyACM0', 1000000, timeout=0.1)
    ...
    def _on_lift(self, msg: String):
        ...
        if c == 'down':
            self.ax_lift_goal = max(0, min(1023, self.ax_lift_goal + step))
            self._dirty_ax_lift = True
            self.arduino.write(b'LIFT_DOWN\n')
        elif c == 'up':
            self.ax_lift_goal = max(0, min(1023, self.ax_lift_goal - step))
            self._dirty_ax_lift = True
            self.arduino.write(b'LIFT_UP\n')
