...
class ArmController(Node):
    def __init__(self):
        ...
        # ---- 추가 구독 ----
        self.sub_arm_manual  = self.create_subscription(String, '/arm/manual',       self.on_arm_manual, 50)
        self.sub_lift        = self.create_subscription(String, '/arm/manual',      self.on_lift, 20)
        self.sub_back_lift   = self.create_subscription(String, '/arm/manual', self.on_back_lift, 20)
        self.sub_gripper     = self.create_subscription(String, '/arm/manual',   self.on_gripper, 20)
    ...
    def on_back_lift(self, msg: String):
        c = msg.data.strip().lower()
        if   c == 'blift_up':       tick = self.back_up_tick
        elif c == 'blift_down':     tick = self.back_down_tick
        elif c == 'blift_down45':     tick = self.back_down_45_tick
        elif c in ('blift_center','home'):
    ...
    def on_lift(self, msg: String):
        c = msg.data.strip().lower()
        if   c == 'flift_down':   tgt = self.x_lift_down_tick
        elif c == 'flift_center': tgt = self.x_lift_center_tick
        elif c == 'flift_up':     tgt = self.x_lift_up_tick
        else: return
